...
map.init(&word, word_entries, rt_countof(word_entries));
map.init(&lens, len_entries, rt_countof(len_entries));
map.init(&poss, pos_entries, rt_countof(pos_entries));
    lz77_if_error_return(lz);
    if (window_bits < 10 || window_bits > 20) { lz77_return_invalid(lz); }
...
if (len < rt_countof(word.entry[0])) {
